...
function OwnEvents() {
  ...
  return events.isPending ? (
    ...
    <EmptyState
      ...
      action={
        <ButtonLink href="/organizer/events/new">Create an event</ButtonLink>
      }
    />
  ) : (
    <div className="grid gap-8">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Stat
          ...
          hint={needApproval.length ? "Hours post when you approve" : undefined}
        />
      </div>
...
/** Organizers see the events they own; admins see every event and who runs it. */
export default function OrganizerPage() {
  ...
  return (
    <Container className="py-8 sm:py-10">
      <PageHeader
        eyebrow="Organize"
        title={admin ? "All events" : "Your events"}
        description={
          admin
            ? "Every event across every organizer, including drafts. A new event goes to the organizer you pick."
            : "Post shifts, watch them fill, mark attendance on the day, and approve hours before you leave the building."
        }
        action={
          <ButtonLink
      ...
      />
      {!user ? <Spinner /> : admin ? <AllEvents /> : <OwnEvents />}
    </Container>
  );
}
